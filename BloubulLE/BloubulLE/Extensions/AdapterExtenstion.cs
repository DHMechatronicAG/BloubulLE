...
namespace DH.BloubulLE.Extensions
{
    public static class AdapterExtenstion
    {
        ...
        /// <param name="cancellationToken">The token to monitor for cancellation requests. The default value is None.</param>
        /// <returns>A task that represents the asynchronous read operation. The Task will finish after the scan has ended.</returns>
        public static Task StartScanningForDevicesAsync(this IAdapter adapter, Func<IDevice, Boolean> deviceFilter,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            return adapter.StartScanningForDevicesAsync(deviceFilter: deviceFilter,
        ...
        public static Task ConnectToDeviceAsync(this IAdapter adapter, IDevice device,
